...
int main()
{
                                ...
                                for (i7 = 0; i7 < 8; i7++)
                                {
                                    ...
                                    for (i = 0; i < 8 && isvalid; i++)
                                        for (j = 0; j < 8 && isvalid; j++)
                                        {
                                            ...
                                            {
                                                if (i + k < 8 && a[i][j] * a[i + k][j])
                                                {
                                                    isvalid = 0;
                                                }
                                                //if (a[i][j] * a[i][j+k] && j+k<8) isvalid = 0;
                                                if (i + k < 8 && j + k < 8 && a[i][j] * a[i + k][j + k])
                                                {
                                                    isvalid = 0;
                                                }
                                                if (i - k >= 0 && j + k < 8 && a[i][j] * a[i - k][j + k])
                                                {
                                                    isvalid = 0;
                                                }
                                            }
    ...
    printf("%s", ctime(&start_time));
    printf("%s\n", ctime(&end_time));
    printf("The total running time is %ld seconds\n", end_time - start_time);
    ec = clock();
    printf("sc = %ld, ec = %ld, Clock ticks: %f", sc, ec, (ec - sc) / (float)CLOCKS_PER_SEC);
